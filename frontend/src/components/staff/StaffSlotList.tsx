...
function StaffSlotCard({ slot }: StaffSlotCardProps) {
	...
	// ✅ ADDED: Expiry detection
	const now = new Date();
	const slotEnd = new Date(`${slot.slot_date}T${slot.end_time}`);
	const isExpired = slotEnd < now;

	const getStatusColor = () => {
		if (isExpired) return "bg-gray-200 text-gray-600 border-gray-300";
	...
	// ✅ NEW: Expired slots NOT clickable
	if (isExpired) {
		return (
			<div className="block bg-gray-50 rounded-2xl border shadow-sm p-5 group opacity-60 cursor-not-allowed">
				{CardContent}
			</div>
		);
	}
